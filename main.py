    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while True:
            ...
            try:
                ...
                try:
                    sqldef.saveData(cursor, conn, "BenchPress", reps)
                    print("벤치 성공")
                except:
                    print("실패")
...
def video_feed_squat():
    return Response(gen_frames_squat(), mimetype='multipart/x-mixed-replace; boundary=frame')


@app.route('/bench')
def video_feed_bench():
    return Response(gen_frames_bench(), mimetype='multipart/x-mixed-replace; boundary=frame')


@app.route('/dead')
def video_feed_dead():
    return Response(gen_frames_dead(), mimetype='multipart/x-mixed-replace; boundary=frame')
